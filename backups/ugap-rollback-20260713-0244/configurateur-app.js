        ...
        let _parcoursUiRefreshTimer = null;
        ...
        function getActiveDevisTemplate() {
        ...
        function scheduleParcoursUiRefresh() {
            if (_parcoursUiRefreshTimer) clearTimeout(_parcoursUiRefreshTimer);
            _parcoursUiRefreshTimer = setTimeout(() => {
                _parcoursUiRefreshTimer = null;
                if (state.step !== 3 || !window.UgapConfiguratorTemplateTree?.renderTemplateTreeStep3) return;
                window.UgapConfiguratorTemplateTree.renderTemplateTreeStep3(state, getTemplateTreeHooks());
            }, 48);
        }
        ...
        function getTemplateTreeHooks() {
            ...
            return {
                ...
                showAllParcoursSlots: displayOptions.showIncludedLines === true,
        ...
        function syncDevisHeaderUi() {
            const opts = getDevisDisplayOptions();
            document.querySelectorAll('[data-devis-display-toggle]').forEach((btn) => {
                const key = btn.getAttribute('data-devis-display-toggle');
                if (key !== 'showIncludedLines') return;
                const active = opts.showIncludedLines === true;
                btn.classList.toggle('is-active', active);
                btn.setAttribute('aria-pressed', active ? 'true' : 'false');
            });
        }

        function onDevisDisplayToggleClick(event) {
            const btn = event?.target?.closest('[data-devis-display-toggle]');
            if (!btn) return;
            const key = btn.getAttribute('data-devis-display-toggle');
            if (key !== 'showIncludedLines') return;
            const next = !getDevisDisplayOptions().showIncludedLines;
            state.devisDisplayOptions = normalizeDevisDisplayOptions({
                ...getDevisDisplayOptions(),
                showIncludedLines: next
            });
            syncDevisHeaderUi();
            scheduleParcoursUiRefresh();
        }
        ...
        window.onDevisDisplayToggleClick = onDevisDisplayToggleClick;
