<?php
/**
 * Mon espace — Agents automatiques (aucune intervention humaine).
 */
...
$page_title = 'Agents automatiques';
$jwt_token = getJWTToken();
$api_base_url = rtrim(getApiBaseUrl(), '/');
$createAgentUrl = url('pages/entity-agent-editor.php') . '?space=user&return=auto';
...
renderConsoleLayoutStart(
    'Agents automatiques',
    'Automatisations sans intervention humaine (poll, cron, webhook → fin du flow).',
...
<div class="card" style="margin-bottom:1rem;">
    ...
    <div class="card-body" style="display:flex; gap:0.75rem; flex-wrap:wrap;">
        ...
        <span class="text-muted small" style="align-self:center;">Chaque modèle crée votre propre agent (modifiable).</span>
    </div>
</div>
    ...
    function createFromTemplate(templateId) {
        ...
        fetch(API + '/templates/' + encodeURIComponent(templateId) + '/create', {
            ...
            .then(function(data) {
                ...
                if (id) {
                    var sep = editorBase.indexOf('?') >= 0 ? '&' : '?';
                    window.location.href = editorBase + sep + 'flowId=' + encodeURIComponent(id);
                    return;
                }
