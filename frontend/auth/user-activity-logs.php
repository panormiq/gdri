<?php
/**
 * Endpoint de lecture des logs d'activité utilisateur
 * Fichier : auth/user-activity-logs.php
 */
...
try {
    ...
    foreach (['event_type', 'user_email', 'entreprise_id', 'from', 'to'] as $k) {
        $v = trim((string) ($_GET[$k] ?? ''));
        if ($v !== '') {
            $params[$k] = $v;
        }
    }
    $url = $apiBase . '/activity-logs/user?' . http_build_query($params);
    $ch = curl_init($url);
    ...
    if ($err || $code < 200 || $code >= 300) {
        jsonResponse(false, 'Erreur API logs utilisateurs');
    }
    $decoded = json_decode((string) $raw, true);
    if (empty($decoded['success'])) {
        jsonResponse(false, $decoded['message'] ?? 'Erreur API logs utilisateurs');
    }
    ...
    error_log('Erreur logs user activity : ' . $e->getMessage());
