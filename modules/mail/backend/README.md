...
- ✅ **Multi-profils SMTP** : Plusieurs adresses d'envoi par module (envoi d'emails)
- ✅ **IMAP (réception)** : Connexion à une boîte mail pour lecture / traitement (via `imap_config`)
- ✅ **Routing intelligent** : Sélection automatique du profil SMTP selon contexte
...
async function init(app, db) {
  ...
  mail.initModule({
    ...
    // (Optionnel) Configuration IMAP pour lecture des emails (ex: OVH)
    imap_config: {
      host: 'pro1.mail.ovh.net',
      port: 993,
      secure: true,
      user: 'newsletter@mon-entite.fr',
      password: process.env.NEWSLETTER_IMAP_PASS,
      mailbox: 'INBOX' // Optionnel, par défaut 'INBOX'
    },
    routing_rules: [
      {
...
### 4. Utilisation du service IMAP (réception)

```javascript
const mailModule = require('../../modules/mail');
const mail = mailModule.getMailService();

// Exemple : tester la connexion IMAP à partir d'une config chargée en DB
const config = await mail.loadConfigFromDB('1234567890abcdef12345678', 'mon-module');
if (config && config.imap_config) {
  const imapService = mail.getImapService();
  const result = await imapService.testConnection(config.imap_config);
