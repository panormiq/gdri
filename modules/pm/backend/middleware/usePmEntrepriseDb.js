/**
 * FICHIER : modules/pm/backend/middleware/usePmEntrepriseDb.js
 * RÔLE : Attache la DB entreprise courante à la requête PM.
 */
...
async function usePmEntrepriseDb(req, res, next) {
  try {
    const user = req.user;
    if (!user) {
    ...
    if (!entrepriseId) {
      try {
        ...
        console.error('❌ PM: erreur chargement entrepriseId:', dbError);
      }
    }
    ...
    console.error('❌ PM: erreur usePmEntrepriseDb:', error);
    return res.status(500).json({
      success: false,
...
module.exports = { usePmEntrepriseDb };
