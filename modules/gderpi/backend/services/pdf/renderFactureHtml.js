/**
 * FICHIER : modules/gderpi/backend/services/pdf/renderFactureHtml.js
 * RÔLE : Génère le document HTML complet d'une facture client.
 */
...
function renderFactureHtml(context) {
  ...
  const dateFacture = formatDateFr(c.factureDate);
  ...
  const positionLabel = String(c.facturePositionLabel || '').trim();
...
<head>
  <meta charset="UTF-8">
  <title>Facture ${esc(c.factureNumero || '')}</title>
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .gderpi-devis-doc__header { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; border-bottom: 2px solid #1e40af; }
    ...
    .gderpi-devis-doc__title { margin: 0; font-size: 26px; font-weight: 800; color: #1e40af; }
    .gderpi-devis-doc__facture-nature { margin-top: 6px; font-size: 13px; font-weight: 600; color: #475569; }
    ...
    table.gderpi-devis-doc__lines th { text-align: left; padding: 8px 6px; background: #1e40af; color: #fff; font-size: 11px; }
    ...
    table.gderpi-devis-doc__totals tr.gderpi-devis-doc__total-ttc td { font-weight: 800; font-size: 14px; border-top: 2px solid #1e40af; }
    ...
    .gderpi-devis-doc__footer-cgv-link { color: #1e40af; font-weight: 600; }
      ...
      <div class="gderpi-devis-doc__sheet-body">
        <header class="gderpi-devis-doc__header">
          ...
          <div class="gderpi-devis-doc__title-block">
            <h1 class="gderpi-devis-doc__title">FACTURE</h1>
            ${positionLabel ? `<div class="gderpi-devis-doc__facture-nature">${esc(positionLabel)}</div>` : ''}
            <div class="gderpi-devis-doc__meta">
              <div><strong>N° facture</strong> ${esc(c.factureNumero || '—')}</div>
              ${dateFacture ? `<div><strong>Date</strong> ${esc(dateFacture)}</div>` : ''}
        ...
        ${devis ? renderDevisPaymentSection(devis) : ''}
      </div>
...
module.exports = renderFactureHtml;
