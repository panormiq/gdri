/**
 * FICHIER : modules/gderpi/frontend/assets/js/boutiques/conditionsVenteDefaults.js
 * RÔLE : Modèles par défaut conditions de vente B2B / B2C (miroir backend).
 */
(function initGderpiConditionsVenteDefaults(global) {
  'use strict';
  ...
  const B2B_KEYS = ['paiementProModes', 'paiementProDelais', 'livraisonPro', 'garantiesPro', 'litigesPro'];
  const B2C_KEYS = ['paiementParticulier', 'retourParticulier', 'livraisonParticulier', 'garantiesParticulier', 'litigesParticulier'];

  global.GderpiConditionsDefaults = {
    all: DEFAULTS,
    b2bKeys: B2B_KEYS,
    b2cKeys: B2C_KEYS,
    pick(keys) {
      const out = {};
      keys.forEach((key) => { out[key] = DEFAULTS[key] || ''; });
      return out;
    },
    b2b() {
      return global.GderpiConditionsDefaults.pick(B2B_KEYS);
    },
    b2c() {
      return global.GderpiConditionsDefaults.pick(B2C_KEYS);
    }
  };
})(window);
