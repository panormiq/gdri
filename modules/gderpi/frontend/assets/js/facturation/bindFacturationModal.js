...
(function initGderpiBindFacturationModal(global) {
  ...
  async function createFactureComplet(cmd) {
    ...
    commande = await global.GderpiBonCommandeClient?.ensureOnCommande?.(fresh.data || cmd);
    if (!commande) return;
  ...
  async function openFacturationModal(cmd) {
    ...
    try {
      ...
      commande = await global.GderpiBonCommandeClient?.ensureOnCommande?.(res.data || cmd);
      if (!commande) return;
